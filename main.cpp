...
int main() {
    try {
        ...
        std::count << "Max weight: " << Car1.getCarMaxWeight() << "kg" << std::endl;
        std::cout << "People capacity: " << Car1.getPeopleCapacity() << std::endl;
        std::count << "Power: " << Car1.getPower() << "hp" << std::endl;
        ...
        std::cout << "People summary weight: " << Car1.getPeopleWeightSum() << std::endl;
        ...
        try { Car InvId(brand, model, year, fuel_type, self_weight, max_weight, people_capacity, power, fuel_amount, fuel_consumption, id);
        } catch (const std::exception& e) { std::cout << "InvId is empty (expected)" << e.what() <<  std::endl; }
    } catch (const std::exception& e) {
